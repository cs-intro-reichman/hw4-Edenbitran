...
public class ArrCharOps {
    ...
    public static char[] subArray(char[] arr, int beginIndex, int endIndex) {
        String subStr = "";
        char[] cSubStr = new char[endIndex - beginIndex];
        for (int i = beginIndex; i < endIndex; i++) {
            subStr += arr[i];
            cSubStr[i - beginIndex] = arr[i];
        }
        //System.out.println(cSubStr); // print the array
        return subStr.toCharArray(); // convert a string to char[] array and return
    }
    ...
    public static long hashCode(char[] arr) {
        ...
        int j = 1;
        for (int i = 0; i < arr.length; i++) {
            sum += arr[i] * (Math.pow(7, (arr.length - j)));
            j++;
        }
        return sum;
    }
    ...
    public static int compareTo(String str1, String str2) {
        ...
        if (str1.length() == str2.length()) { //if the length is equal
            for (int i = 0; i < str1.length(); i++) {
                ...
                if (str1.charAt(i) != str2.charAt(i)) { // return -1 if str1 comes first and 1if str2 comes first
        ...
        if (str1.length() != str2.length()) {
            for (int i = 0; i < str1.length(); i++) {
                ...
                if (i + 1 == str1.length() && i < str2.length()) {
                    return -1;
                }
            }
            return result;
